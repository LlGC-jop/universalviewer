...
export class EbookCenterPanel extends CenterPanel<Config['modules']['ebookCenterPanel']> {
  private _cfi: string;
  ...
  async create(): Promise<void> {
    ...
    this._ebookReader.addEventListener(
      'loadedNavigation',
      (e: any) => {
        this.extensionHost.publish(EbookExtensionEvents.LOADED_NAVIGATION, e.detail);
      },
      false,
    );
    ...
    this._ebookReader.addEventListener(
      ...
      (e: any) => {
        ...
        this.extensionHost.publish(EbookExtensionEvents.CFI_FRAGMENT_CHANGE, this._cfi);
      },
      false,
    );
    ...
    this.extensionHost.subscribe(EbookExtensionEvents.ITEM_CLICKED, (href: string) => {
      this._nextState({
        cfi: href,
      });
    });

    this.extensionHost.subscribe(EbookExtensionEvents.CFI_FRAGMENT_CHANGE, (cfi: string) => {
      Async.waitFor(
        () => {
